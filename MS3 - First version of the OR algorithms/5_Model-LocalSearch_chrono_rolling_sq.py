...
#test parameters
seeds = [25, 200, 972, 343, 5049, 8090, 37073, 15503, 96350, 66301]
        ...
        for k in range(n_iter):
            ...
            while tf < 3600:
                ...
                if iter_w_imp == limits[ttype][ilim]:
                    ...
                    iter_w_imp = 0
                    delta = round(coeff * nP)
                    w = np.random.randint(nP-delta) #punto random
                print(f'Iteration: {index}, Coeff: {coeff}')

                ### Selection of observations to fix ###
            ...
            g = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_rolling_rsVND_{gra}_{L}_{nCat}.txt", "a")
            ...
            f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_rolling_rsVND(sol)_{gra}_{L}_{nCat}.txt", "a")
            f.write(f'Objective Value = {obj} \n')
            for i in selected:
                f.write("%d\n" % i)
            f.close()

        f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_rolling_rsVND_{gra}_{L}_{nCat}.txt", "a")
