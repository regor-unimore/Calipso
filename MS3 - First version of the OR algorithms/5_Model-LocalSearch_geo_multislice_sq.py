        ...
        for ttype in ['VHR', 'HR']:
            ...
            for k in range(n_iter):
                ...
                while tf < 3600:
                    ...
                    alpha = [np.random.random()*(round_angle-delta) for _ in range(5)]  # random rotation
                    #alpha = np.random.random() * (round_angle - delta)
                    #print(f'range: [{alpha}, {alpha+delta}]')

                    counter = 0
                    for i in range(nP):
                        if alpha[0] <= angle[i] < alpha[0] + delta or alpha[1] <= angle[i] < alpha[1] + delta or alpha[2] <= angle[i] < alpha[2] + delta \
                                or alpha[3] <= angle[i] < alpha[3] + delta or alpha[4] <= angle[i] < alpha[4] + delta:
                    ...
                    if obj_prec == obj:
                        ...
                        ilim = 0  # VND
                        coeff = coeff_0  # VND
                        delta = coeff * round_angle / n_slices  # VND
                ...
                g = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_geo_multisliceVND_{gra}_{L}_{nCat}.txt", "a")
                ...
                f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_geo_multisliceVND(sol)_{gra}_{L}_{nCat}.txt", "a")
                f.write(f'Objective Value = {obj} \n')
                for i in selected:
                    f.write("%d\n" % i)
                f.close()

            f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_geo_multisliceVND_{gra}_{L}_{nCat}.txt", "a")
