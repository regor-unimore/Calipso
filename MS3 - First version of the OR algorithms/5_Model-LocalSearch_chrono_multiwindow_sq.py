...
#test parameters
seeds = [7, 100, 672, 203, 4346, 8799, 17873, 15593, 78380, 96704]
...
a = {0.15: 1, 0.25: 2, 0.35: 6, 0.45: 12}
nCat = 4
#L = 134
N = 1
...
for gra in [1000, 2000, 3000, 5000, 10000]:
    ...
    for L in [134]:#,134,268]:
        ...
        with open(f"4_Solutions/H3/Heuristic3Solution_chrono_{gra}_{L}_{nCat}.txt", "r") as file:
        ...
        ######################### MODEL EXECUTION ###############################
        #ttype = 'HR'  # 'HR, 'MR', 'LR', 'VLR'

        if gra == 10000:
            ttype = 'VHR'  # 'VHR', 'HR, 'MR', 'LR', 'VLR'
        else:
            if gra == 5000:
                ttype = 'HR'
            else:
        ...
        for k in range(n_iter):
            ...
            ilim = 0
            coeff = coeff_0
            delta = round(coeff * nP / n_windows)
            tf = 0
            test = []
            ...
            while tf < 3600:
                ...
                #time_limit = min(300, np.ceil(a[coeff] * nPI[gra] / 10000))
                #model.setParam("TimeLimit", time_limit)
                #print(f'Time Limit: {time_limit}')

                ### Selection of random observations to unfix ###
                w = [np.random.randint(0, nP - delta) for _ in range(n_windows)]
                range_unfix = [range(i, i+delta) for i in w]

                for i in range(nP):
                    if i in range_unfix[0] or i in range_unfix[1] or i in range_unfix[2] \
                            or i in range_unfix[3] or i in range_unfix[4]: #variabili libere
                        isPositionActive[i].lb = 0
                        isPositionActive[i].ub = 1
                    else:
                        isPositionActive[i].lb = selected[i]
                        isPositionActive[i].ub = selected[i]
                ...
                if obj_prec == obj:
                    ...
                    #ilim = 0  # VND
                    #coeff = coeff_0  # VND
                    #delta = round(coeff * nP / n_windows)  # VND
            ...
            g = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_multiwindow_{gra}_{L}_{nCat}_score.txt", "a")
            ...
            f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_multiwindow(sol)_{gra}_{L}_{nCat}_score.txt", "a")
            f.write(f'Objective Value = {obj} \n')
            for i in selected:
                f.write("%d\n" % i)
            f.close()

        f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_multiwindow_{gra}_{L}_{nCat}_score.txt", "a")
