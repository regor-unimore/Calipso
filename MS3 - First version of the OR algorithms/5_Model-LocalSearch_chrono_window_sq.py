...
for gra in [1000, 2000, 3000]:  # , 5000, 3000, 2000, 1000]:
    ...
    for L in [134]:#,134,268]:
        ...
        for k in range(n_iter):
            ...
            while tf < 3600:
                ...
                w = np.random.randint(0, nP - delta)
                range_unfix = range(w, w+delta)

                for i in range(nP):
                    if i in range_unfix: #variabili libere
            ...
            g = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_randomVND_{gra}_{L}_{nCat}.txt", "a")
            ...
            f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_randomVND(sol)_{gra}_{L}_{nCat}.txt", "a")
            f.write(f'Objective Value = {obj} \n')
            for i in selected:
                f.write("%d\n" % i)
            f.close()

        f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_chrono_randomVND_{gra}_{L}_{nCat}.txt", "a")
