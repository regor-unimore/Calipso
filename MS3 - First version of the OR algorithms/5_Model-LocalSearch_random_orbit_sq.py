...
seeds = [8, 51, 739, 291, 846, 5099, 12073, 1093, 30380, 81004]
...
with open("2_RawDataFuture/orbit_number_2023_2025.dat", "r") as file:
    ...
    for L in [134]:#,67,134,268]:
        ...
        for k in range(n_iter):
            ...
            while tf < 3600:
                ...
                orbit_fix = {orbit[i]: 0 for i in range(nP)}
                ...
                for o in orbit_fix.keys():
                    if orbit_fix_r[idx] < coeff:
                        orbit_fix[o] = 1
                    idx += 1
                #print(sum(orbit_fix.values()))

                for i in range(nP):
                    if orbit_fix[orbit[i]] == 1:
                        isPositionActive[i].lb = selected[i]
                        isPositionActive[i].ub = selected[i]
                    else:
                        isPositionActive[i].lb = 0  # selected[i]
                        isPositionActive[i].ub = 1  # selected[i]
            ...
            g = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_random_orbitVND_{gra}_{L}_{nCat}.txt", "a")
            ...
            f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_random_orbitVND(sol)_{gra}_{L}_{nCat}.txt", "a")
            f.write(f'Objective Value = {obj} \n')
            for i in selected:
                f.write("%d\n" % i)
            f.close()

        f = open(f"4_Solutions/{ttype}_{coeff_0}_{step}_random_orbitVND_{gra}_{L}_{nCat}.txt", "a")
